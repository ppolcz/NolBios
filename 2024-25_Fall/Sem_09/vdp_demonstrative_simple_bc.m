...
% Nb = sym(P_affine_annihilator([1;phi],x,'sym',1))
Nb = [
    x1, -1,  0,  0,  0
    x2,  0, -1,  0,  0
     0, x2,  0, -1,  0
...
SHOULD_BE_ZERO = norm(double(simplify(  Nb * [1;phi]  )))
...
% Boundary conditions
tau = sdpvar(1,X_NrF);
Pb = @(alpha) blkdiag(-alpha,P);
for i = 1:X_NrF

    S = null([1 -ak(i,:) zeros(1,m-2)]);

    L_b1 = sdpvar(sb,mb);
    L_b2 = sdpvar(sb,mb);

    for j = 1:2
        Nij = Nb_fh(X_v(X_fci(i,j),:)');
        CONS = [ CONS
            S'*( Pb(1) + L_b1*Nij + Nij'*L_b1' )*S >= 0
            S'*( Pb(tau(i)) + L_b2*Nij + Nij'*L_b2' )*S <= 0
            ];

    end
end

OBJ = optimize(CONS,sum(tau));
P = double(P);
