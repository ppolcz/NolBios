...
x0 = [-1.08846,-0.669485,-0.911298];
x0 = [-0.87053,-0.65111,-1.18765];

% solve the differential equation system
[t,x] = ode89(f_ode,[0,3*Tp],x0); 
...
u_vals = sort([
    linspace(-1.22,-1.17,71) ...
    linspace(-1.180,-1.176,51) ...
    linspace(-1.177,-1.176,51) ...
    linspace(-1.17647,-1.17645,31)
    ]);
...
ylabel('real(eig)')
zlabel('imag(eig)')
legend('S1 (eig1&2)','S2 (eig1&2)','S3 (eig1&2)')
...
u_vals = linspace(-1.22,-1.17,51);
u_vals = linspace(-1.5,-1,51);
...
for u_val = u_vals
    ...
    ax = nexttile;
    hold on, box on, grid on;
    fimplicit(dV_fh,Vw_range,'Color',[1,0,0],'LineWidth',2)
    fimplicit(dw_fh,Vw_range,'Color',[1,0,0],'LineWidth',2)
    ...
    [VV,ww] = meshgrid(linspace(V_range(1),V_range(2),50),linspace(w_range(1),w_range(2),250));
    dVV = dV_fh(VV,ww);
    dww = dw_fh(VV,ww);
