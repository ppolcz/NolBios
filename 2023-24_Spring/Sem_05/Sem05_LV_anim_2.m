...
ind = 0;
for eta_val = 1:0.005:2
    ind = ind + 1;
        
    odefun = @(t,x) f_fh(x,mu_val,eta_val);
    jacfun = @(x) J_fh(x,mu_val,eta_val);
    
    [trajectories,starts,ends,SEP_set,UEP_set] = phase_portrait(odefun,jacfun, ...
        'EP',double(subs(Eq,[mu,eta],[mu_val,eta_val])), ...
    ...
    f1_val = f1_fh(xx,yy,mu_val,eta_val);
    f2_val = f2_fh(xx,yy,mu_val,eta_val);
    ...
    xlim([-0.5,LimX])
    ylim([-0.5,LimY])
    title(str + ", $\mu = " + num2str(mu_val) + "$, $\eta = " + num2str(eta_val) + "$",'Interpreter','latex')
    ...
    fimplicit(@(x,y) f1_fh(x,y,mu_val,eta_val),[-0.5,LimX,-0.5,LimY]);
    fimplicit(@(x,y) f2_fh(x,y,mu_val,eta_val),[-0.5,LimX,-0.5,LimY]);
